...
constexpr int movie_ui_state(int requested, bool presenting) noexcept {
    return presenting && (requested == 0x2B || requested == 0x2D || requested == 0x2F) ? 0x21
                                                                                       : requested;
}
constexpr bool can_chain_movies(Owner current,
                                Owner next,
    ...
    return current.session && current.generation && current == next && current == firstCompleted
           && previous > 0 && requested == previous + 1 && state == Status::complete;
}
