...
struct OrbitObservation {
    bool exactOwner{}, sourceSelected{}, orbitSelected{}, complete{}, ready{};
...
struct OrbitReturn {
    ...
    void arm(std::uint64_t now) {
        stage = OrbitStage::completion;
        began = now;
    }
    bool active() const {
        return stage != OrbitStage::idle && stage != OrbitStage::finished;
    }
    OrbitAction observe(std::uint64_t now, const OrbitObservation& s) {
        if (!active()) return OrbitAction::none;
        auto end = [this](OrbitAction a) {
            stage = OrbitStage::finished;
            return a;
        };
        if (now - began > 90000) return end(OrbitAction::timedOut);
        if (stage == OrbitStage::completion) {
            if (!s.exactOwner || !s.sourceSelected || s.region < 0 || s.step != 38)
        ...
        if (stage == OrbitStage::selection) {
            ...
            if (!s.sameWorld || (!s.sourceSelected && !s.orbitSelected))
                return end(OrbitAction::canceled);
            if (s.orbitSelected && s.ready && s.pendingStep == 38) {
                stage = OrbitStage::leaving;
                return OrbitAction::cleanup;
            }
        } else if (s.step == 38 && !s.sameWorld)
            return end(OrbitAction::canceled);
