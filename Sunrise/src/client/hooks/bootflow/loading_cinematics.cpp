...
using ActivityReader = scripted_presentation::ActivityReader;
...
bool __fastcall loading_cinematics_suppressed() noexcept {
    ...
    // can still name an old activity after the client selects another destination.
    const auto registration = scripted_presentation::current();
    if (registration
        && registration->config.suppressLoadingCinematics) {
        return true;
    }
...
StageResult stage_loading_cinematics(hooking::detour::Spec& spec) noexcept {
    ...
    scripted_presentation::activity_reader(g_activityReader);
