...
#include "../scripted_presentation/config_rules.h"
#include "../scripted_presentation/resources.h"
#include "internal.h"
...
std::uint32_t* effect_template(void* component,
                               const scripted_presentation::EffectAttachment& effect) noexcept {
    __try {
        auto* self = static_cast<std::byte*>(component);
        if (!scripted_presentation::effect_matches(effect,
                                                   scripted_presentation::current_region(),
...
scripted_presentation::EffectAttachment attachment_rule(void* component) noexcept {
    ...
    for (unsigned i = 0; i < config.effectCount; ++i) {
        ...
        if (effect.region == scripted_presentation::current_region()
...
bool __fastcall attach(void* component, std::uint32_t actor) noexcept {
    ...
    if (!scripted_presentation::effect_resident(effect.replacement)) {
