...
void __fastcall draw_widget(void* widget, void* context, void* commands) noexcept {
    const bool presenting = scripted_presentation::presenting();
        ...
        if (role == 0x12) {
            if (!reported) {
                ...
                core::log::writef(
                    ...
                    core::log::Level::info,
                    "ev=movie result=hud_draw_suppressed role=%d asset=%08X name=%08X",
                    role,
                    asset,
                    name);
                reported = true;
...
StageResult stage_movie_hud(hooking::detour::Spec& spec) noexcept {
    ...
    constexpr auto windowDrawSig = signature<signature_length(
        "4C 8B C7 48 8D 54 24 70 48 8B CE E8 ? ? ? ? 48 8B CF E8 ? ? ? ?")>(
        "4C 8B C7 48 8D 54 24 70 48 8B CE E8 ? ? ? ? 48 8B CF E8 ? ? ? ?");
    constexpr auto widgetSig = signature<signature_length(
        "40 57 41 55 41 56 48 83 EC 30 48 8B F9 4D 8B F0 0F B7 49 1E 4C 8B EA")>(
        "40 57 41 55 41 56 48 83 EC 30 48 8B F9 4D 8B F0 0F B7 49 1E 4C 8B EA");
    const auto* caller = scan_main_image_unique(windowDrawSig, "movie_hud_window_call");
    auto* target = scan_main_image_unique(widgetSig, "movie_hud_widget_draw");
...
void publish_movie_hud(const hooking::detour::Handle& value) noexcept {
    handle = value;
    core::log::write(core::log::Channel::client,
                     core::log::Level::info,
                     value.attached ? "ev=movie result=hud_filter_attached"
                                    : "ev=movie result=hud_filter_attach_failed");
}
