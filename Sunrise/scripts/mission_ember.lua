...
return {
    ...
    on_event_cinematic_started = function(c, s, e)
        if s:variable("ember.ending") then later().dispatch("cinematic_started", c, s, e) end
    end,
    on_event_cinematic_terminated = function(c, s, e)
        if s:variable("ember.ending") then later().dispatch("cinematic_terminated", c, s, e)
        else opening.terminated(c, s, e) end
    end,
    on_event_cinematic_skip_requested = function(c, s, e)
        if s:variable("ember.ending") then later().dispatch("cinematic_skip_requested", c, s, e)
        else opening.terminated(c, s, e) end
    end,
    on_event_client_state_changed = function(context, state, event)
        if state:variable("ember.ending") then later().client(context, state, event); return end
    ...
    on_event_effect_result = function(c, s, e)
        if s:variable("ember.ending") then later().dispatch("effect", c, s, e)
        else wipe.effect(c, s, e) end
    end,
}
