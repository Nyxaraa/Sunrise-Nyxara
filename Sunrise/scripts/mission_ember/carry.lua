...
return function(api, tag, pickup, receptacle, on_pickup, on_deposit)
    ...
    local function complete(c, s)
        if s:variable(key("done")) or not s:variable(key("notified"))
            or s:variable(key("accepted_generation")) ~= generation(s) then return end
    ...
    function obj.interaction(c, s, e)
        ...
        if api.matches(c, e, receptacle) and e.generation == s:variable(key("receptacle_generation")) then
            -- Retain accepted native use while a pickup/ownership receipt is still in flight.
            c:set_variable(key("accepted_generation"), generation(s))
            complete(c, s)
    ...
    function obj.timer(c, s, e)
        ...
        c:clear_variable(key("accepted_generation"))
