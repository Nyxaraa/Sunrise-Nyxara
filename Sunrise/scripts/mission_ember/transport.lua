...
-- Authored group resolved from the live Harvester action table.
local EXIT_ACTION = 0x7B0D3643
local EXIT_GROUP = 0x07EBF354
return function(mission, passengers)
    ...
    local function doors(context, state, ship, revision, open)
        context:slot(ship.member):set_actor_channels{generation = generation(state), revision = revision,
            channels = {{channel = 0x80296344, value = open and 1 or 0}}}
    end
    ...
    local function depart(context, state, index)
        ...
        doors(context, state, ships[index], 3, false)
    ...
    local function unload(context, state, index)
        ...
        if #cargo > 0 then
            doors(context, state, ships[index], 2, true)
    ...
    return {
        ...
        start = function(context, state)
            ...
            for _, ship in ipairs(ships) do
                path(context, state, ship, 1)
                doors(context, state, ship, 1, false)
            end
        end,
            ...
            for index, ship in ipairs(ships) do
                ...
                if event.registry_key == member.registry_key and event.slot_type == member.slot_type
                    ...
                    if not event.dead and event.revision == 2 and event.path_state == 1
                        ...
                        member:play_actor_action{generation = generation(state), revision = 3, group = EXIT_GROUP, action = EXIT_ACTION}
                    end
