...
return function(m, a, ending)
    ...
    local function hazards(c, s, mode)
        ...
        -- One native thermal attachment owns both phases; revising it detaches the old filter.
        a.objects(c, {"SUNBURN_DAMAGE_OBJECT"}, false)
        local filter = "REACTOR_MOTHER_BRAIN_HOT_PIPES_OBJECT_FILTER_80B3C09F"
        local selection = {players = true, inside_any = {
            ...
            a.slot(c, "SLOT_0008_80B3C09F")}}
        if mode == "escape" then
            filter = "AOD_REACTOR_RAIL_TOP_OBJECT_FILTER"
            selection = {players = true, inside = a.slot(c, "SLOT_019E")}
        end
        a.effect(c, s, "REACTOR_MOTHER_BRAIN_HOT_PIPES_THERMAL_HOP_ON",
            filter, selection, mode == "climb" or mode == "escape")
    end
